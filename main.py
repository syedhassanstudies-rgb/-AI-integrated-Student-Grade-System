...
def print_transcript(transcript: Optional[dict]) -> None:
    ...
    print(f"  {'-' * 40}")
    for g in grades:
        print(f"  {g.total_marks:<20} {g.marks_obtained}/{g.total_marks}")
        print(f"  {'-' * 40}")
...
def handle_add_grade() -> None:
    student_id = get_valid_int(  "Student I:  ")
...
def handle_delete_student() -> None:
    student_id = get_valid_int(  "  Studen ID:  ")
